package com.worldline.android.headyio.modules.most_shared;
...
public class MostSharedFragment extends Fragment
{
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		if(null==ProductsSingleton.getInstance().getMostSharedProductList() && null==ProductsSingleton.getInstance().getMostSharedViewsList())
		{
			doDataProcessingAndShowProducts();
		}
		else
		{
			sortedProductsList= (LinkedList<Product>) ProductsSingleton.getInstance().getMostSharedProductList();
			viewsList= (LinkedList<Integer>) ProductsSingleton.getInstance().getMostSharedViewsList();
			showDataInListView();
		}
		...
		new AsyncTask<Void, Void, Void>()
		{
			...
			protected Void doInBackground(Void... voids)
			{
				try
				{
					...
					{
						if (jsonArray.getJSONObject(i).getString("ranking").equals("Most ShaRed Products"))
						{
					...
					for (int i = 0; i < jsonArray1.length(); i++)
					{
						...
						obj2.put(obj.get("id"), obj.get("shares"));
					}
					...
					ProductsSingleton.getInstance().setMostSharedProductList(sortedProductsList);
					ProductsSingleton.getInstance().setMostSharedViewsList(viewsList);
	...
	public void showDataInListView()
	{
		try
		{
			ProductAdapter mAdapter = new ProductAdapter(getFragmentManager(),sortedProductsList,viewsList,"Shared");
